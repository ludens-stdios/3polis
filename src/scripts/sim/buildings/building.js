...
export class Building extends SimObject {
      ...
      switch (status) {
        case BuildingStatus.NoPower:
          ...
          this.statusIcon.material.map =
            window.assetManager.statusIcons[BuildingStatus.NoPower];
          break;
  ...
  toHTML() {
    let html = `
      <div class="info-heading">Edificio</div>
      ...
      <br>
      <span class="info-label">Acceso a carretera </span>
      <span class="info-value">${
        this.roadAccess.value == true ? "✅" : "❌"
      }</span>
      <br>`;

    if (this.power.required > 0) {
      html += `
        <span class="info-label">Potencia (kW)</span>
